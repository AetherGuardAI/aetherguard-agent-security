"""
AetherGuard + CrewAI + Anthropic Claude Integration Example
...
# AetherGuard Proxy URL - optional Anthropic-compatible gateway for LLM calls
...
ANTHROPIC_BASE_URL = os.getenv("ANTHROPIC_BASE_URL", AETHERGUARD_PROXY_URL)
...
def _anthropic_model_name(model: str) -> str:
    """Return the LiteLLM/CrewAI provider-qualified Anthropic model name."""
    return model if model.startswith("anthropic/") else f"anthropic/{model}"


def validate_anthropic_config() -> None:
    """Fail fast with a clear message when Claude credentials are missing."""
    if not ANTHROPIC_API_KEY or ANTHROPIC_API_KEY == "your_anthropic_api_key":
        raise RuntimeError(
            "Set ANTHROPIC_API_KEY to a valid Anthropic API key before running this example."
        )
...
# ─────────────────────────────────────────────────────────────────────────────
# CrewAI Agents (Claude LLM calls routed through Anthropic/AetherGuard Proxy)
# ─────────────────────────────────────────────────────────────────────────────

# LLM instance configured for Anthropic Claude. CrewAI uses LiteLLM's Anthropic
# provider, and ANTHROPIC_BASE_URL can point at an Anthropic-compatible
# AetherGuard proxy endpoint when proxy routing is enabled.
aetherguard_llm = LLM(
    model=_anthropic_model_name(ANTHROPIC_MODEL),
...
def main():
    print("=" * 70)
    print("  AetherGuard + CrewAI + Anthropic Claude Integration Example")
